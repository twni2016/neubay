"""
Adapted from https://github.com/proroklab/memoroids/blob/main/buffer.py
"""
...
class ReplayBufferBase:
    ...
    def __init__(
        self, buffer_size: int, schema: Dict[str, np.shape], preallocate_memory=True
    ):
...
class MarkovBuffer(ReplayBufferBase):
    """A standard replay buffer for Markov RL training."""
...
class TapeBuffer(ReplayBufferBase):
    """A tape-based buffer for recurrent RL training."""
    ...
    def sample(self, size: int, key: jax.random.PRNGKey) -> Dict[str, np.ndarray]:
        ...
        rng = np.random.default_rng(jax.random.bits(key).item())
        count = 0
        # print("sample", len(self.episode_starts))
        while count < size:
            # NOTE: fixed the issue of sampling range
            start_idx = rng.integers(len(self.episode_starts))
            start = self.episode_starts[start_idx]

            if start_idx == len(self.episode_starts) - 1:
                end = self.ptr
            else:
            ...
            # print(start_idx, start, end, count)
    ...
    def add(self, **data) -> None:
        ...
        while (self.size + batch_size) > self.max_size:
            ...
            # Handle case where indices wrap around, e.g. [8, 0, 2, 4] case
        ...
        # Add new start indices to episode_starts
        idx = np.arange(self.ptr, self.ptr + batch_size) % self.max_size
        new_starts = (self.ptr + np.flatnonzero(data[self.start_key])) % self.max_size
        self.episode_starts.extend(new_starts.tolist())
...
def test_add_sample():
    ...
    assert (
        np.concatenate([data["start"], data2["start"]]) == b.data["start"][:7]
    ).all()
...
def test_wraparound():
    ...
    data0 = {"a": np.arange(2), "start": np.array([True, False])}
    data1 = {"a": 2 + np.arange(2), "start": np.array([True, False])}
    data2 = {"a": 4 + np.arange(2), "start": np.array([True, False])}
    b.add(**data0)
    b.add(**data1)
    b.add(**data2)
    ...
    # Check flags
    assert np.all(
        expected_start == b.data["start"]
    ), f"expected: {expected_start}\nactual: {b.data['start']}"
    # Check indices
    assert np.all(
        np.where(expected_start)[0] == b.episode_starts
    ), f"expected: {np.where(expected_start)[0]}\nactual: {b.episode_starts}"
    # Check size
...
def test_wraparound2():
    ...
    # [0, 1, ..., 38, 39]
    datas = [{"a": i * 2 + np.arange(2), "start": np.array([1, 0])} for i in range(20)]
    for d in datas:
        b.add(**d)
